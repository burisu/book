...
task :rights => :environment do
  ...
  rights = (File.exist?(MandateNature.rights_file) ? YAML.load_file(MandateNature.rights_file) : {})
  ...
  for right, attributes in rights
    attributes['actions'].each_index do |index|
      ...
      unless ref[controller] and ref[controller].include?(action)
  ...
  File.open(MandateNature.rights_file, "wb") do |file|
    file.write code
  end
