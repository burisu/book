class CreateProduct < ActiveRecord::Migration
  ...
  def self.up
    ...
    execute "INSERT INTO sales (number, state, comment, client_id, client_email, amount, created_on, payment_id, sid) SELECT 'V'||subscriptions.number, state, note, person_id, people.email, subscriptions.amount, subscriptions.created_at::DATE, payments.id, subscriptions.id FROM subscriptions JOIN people ON (subscriptions.person_id=people.id) LEFT JOIN payments ON (payments.sid=subscriptions.id)"
    ...
    execute "INSERT INTO products (name, amount, subscribing, subscribing_started_on, subscribing_stopped_on) VALUES ('Adhésion 1 an', 10.5, true, '2009-09-01', '2010-10-31')"
    execute "INSERT INTO sale_lines (sale_id, product_id, name, description, unit_amount, quantity, amount, sid) SELECT id, 1, 'Adhésion 1 an', 'Année 2010', CASE WHEN amount/10.5 = ROUND(amount/10.5) AND amount > 10.5 THEN 10.5 ELSE amount END, CASE WHEN amount/10.5 = ROUND(amount/10.5) AND amount > 10.5 THEN amount/10.5 ELSE 1 END, amount, sid FROM sales"
